...
void main (int argc, char *argv[])
{
  ...
  disksize = disk_size();
  Printf("fdisk -- Disk Size in bytes: %d\n", disksize);

  diskblocksize = disk_blocksize();
  Printf("fdisk -- Logical Disk Block Size in bytes: %d\n", diskblocksize);

  num_filesystem_blocks = (disksize / FDISK_BLOCKSIZE);
  Printf("fdisk -- Number of File System Blocks: %d\n", num_filesystem_blocks);

  Printf("fdisk -- Size of inode: %d\n", sizeof(dfs_inode));
  ...
  fbv[j/32] |= 1<<(j%32);
  j++;
  Printf("fdisk -- 1 file system block used for MBR/Superblock\n");
  ...
  Printf("fdisk -- %d file system blocks used for inodes\n", (64*FDISK_NUM_INODES/FDISK_BLOCKSIZE));
  ...
  Printf("fdisk -- %d file system blocks used for free block vector\n", (num_filesystem_blocks/FDISK_BLOCKSIZE/8));
  ...
  Printf("fdisk -- fdisk (%d): Formatted DFS disk for %d bytes.\n", getpid(), disksize);
