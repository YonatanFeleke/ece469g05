	     ...
	     uint32 *trapArgs)
{
  ...
  uint32	args[4];
  int	intrs;
  unsigned int   pid;  // JSM variable for current process id
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    // JSM added GET_PID handler
    ////////////////////
    case TRAP_GET_PID:
      dbprintf ('t', "Got a getpid trap!\n");
      pid = GetCurrentPid();
      ProcessSetResult (currentPCB, pid);
      break;
    ////////////////////

    // JSM added PUT_CHAR handler
    ////////////////////
    case TRAP_PUT_CHAR:
      dbprintf ('t', "Got a putchar trap!\n");
      MemoryCopyUserToSystem (currentPCB, trapArgs, args, sizeof(uint32));
      PutChar((char)args[0]);
      break;
    ////////////////////
