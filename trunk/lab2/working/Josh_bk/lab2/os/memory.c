...
uint32 MemoryTranslateUserToSystem (PCB *pcb, uint32 addr) {
	...
	dbprintf('m', "Address to translate: 0x%.8X\n", addr);

	offset = addr & (MEM_PAGE_SIZE-1); // lower 12 bits

	dbprintf('m', "Offset: 0x%.8X\n", offset);

	idx = addr >> (MEM_L1FIELD_FIRST_BITNUM);// get rid of lower 12 bit offset

	dbprintf('m', "Index: 0x%.8X\n", idx);

	pte_idx = pcb->pagetable[idx];

	dbprintf('m', "PTE_IDX: 0x%.8X\n", pte_idx);

	if ((pte_idx & MEM_PTE_VALID) == 0) //not valid
	{
		printf("INVALID PAGE TRANSLATION!\n");
		while(1);
	...
	dbprintf('m', "Translated Address: 0x%.8X\n", rtnAddress);
...
int MemoryPageFaultHandler(PCB *pcb) {
  ...
  printf("PAGE FAULT!!  HANDLER NOT CREATED YET!!\n");
  while(1);
}
