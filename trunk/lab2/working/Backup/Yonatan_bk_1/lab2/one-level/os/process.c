...
void ProcessFreeResources (PCB *pcb) {
  ...
  dbprintf ('p', "Free Resources NOT DONE YET!\n");
  while(1);
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  for(i = 0; i < 4; i++)
  {
		...
		dbprintf('p', "Allocating physical page #%d (Address 0x%.8X) for process virtual page #%d (data/code)\n\n", newpage, (newpage*MEM_PAGE_SIZE), i);
  ...
  // Allocate page for user stack
	...
	dbprintf('p', "Allocating physical page #%d (Address 0x%.8X) for process virtual page #%d (user stack)\n\n", newpage, (newpage*MEM_PAGE_SIZE), MEM_L1_PAGE_TABLE_SIZE-1);
	pcb->pagetable[MEM_L1_PAGE_TABLE_SIZE-1] = ((newpage*MEM_PAGE_SIZE) | MEM_PTE_VALID);
  ...
  // Allocate page for system stack
	...
	dbprintf('p', "Allocating physical page #%d (Address 0x%.8X) for process system stack\n\n", newpage, (newpage*MEM_PAGE_SIZE));
	pcb->sysStackArea = newpage * MEM_PAGE_SIZE;
	stackframe = (uint32 *)(pcb->sysStackArea + (MEM_PAGE_SIZE-4));
	dbprintf('p', "Initializing system stack pointer to 0x%.8X\n\n", (uint32)stackframe);
