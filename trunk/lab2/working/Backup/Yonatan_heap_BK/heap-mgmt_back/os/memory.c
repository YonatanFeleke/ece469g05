...
void MemoryModuleInit() {
	...
	// TODO L2 Page table initializations
	// This makes one L2 to be shared among all other processes but addressed differently
	// Should have a level2 in PCB and be initialized in process fork
	// ------DONE--------
...
//
//----------------------------------------------------------------------
// TODO5 CHANGE FOR 2 LEVEL DONE: Memory Translate
...
//	MemoryAllocateL2PT
//
//	Allocates a level 2 page table in memory, returns -1 if unsuccessful
//		, or the allocated level 2 page table number if successful.
//
//----------------------------------------------------------------------
...
//
//		allocate a memory block of size memsize in the heap space and
...
void *malloc(PCB *pcb,int memsize){
	...
	if ((memsize % 8) == 0)	{
		...
		Addr_8byte = memsize/8 +1;
	}
	new_Virtual_Address = MemoryHeapAllocatePage(Addr_8byte);
...
//
//		free the heap block identified by ptr.
//---------------------------------------------------------------------
int mfree(PCB *pcb,void *ptr){
	int bytesFreed;
	bytesFreed = pcb->HeapPtrSizes[((uint32 )ptr & MEM_WORD_ADRESS_MASK)];
	return(bytesFreed);
}


uint32 *MemoryHeapAllocatePage(uint32 Addr_8byte){
	return(0);
}
