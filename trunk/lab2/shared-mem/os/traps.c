  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    case TRAP_MFREE:
      ...
      case TRAP_SHARE_CREATE_PAGE:
        ihandle = GetIntFromTrapArg(trapArgs, isr & DLX_STATUS_SYSMODE);
        ihandle = shmget(ihandle);
        ProcessSetResult(currentPCB, ihandle); //Return handle
        break;
      case TRAP_SHARE_MAP_PAGE:
        ihandle = (int)GetIntFromTrapArg(&trapArgs[0], isr & DLX_STATUS_SYSMODE);
        ihandle2 = (int)GetIntFromTrapArg(&trapArgs[1], isr & DLX_STATUS_SYSMODE);
        dbprintf('S',"Passing Arguemnts currentPCB= 0x%.8X ihandle= %d ihandle2= 0x%.8X\n",(int)currentPCB,ihandle,ihandle2);
        dbprintf('S',"Calling: shmat(0x%.8X, %d, 0x%.8X)\n",(int)currentPCB,ihandle,ihandle2);
        ihandle = shmat(currentPCB, ihandle,ihandle2);
        ProcessSetResult(currentPCB, ihandle); //Return handle
        break;
      case TRAP_SHARE_UNMAP_PAGE:
          ihandle = GetIntFromTrapArg(trapArgs, isr & DLX_STATUS_SYSMODE);
          ihandle = shmdt(currentPCB, (void *)ihandle);
          ProcessSetResult(currentPCB, ihandle); //Return handle
          break;
      case TRAP_SHARE_DELETE_PAGE:
          ihandle = GetIntFromTrapArg(trapArgs, isr & DLX_STATUS_SYSMODE);
          ihandle = shmrm(ihandle);
          ProcessSetResult(currentPCB, ihandle); //Return handle
          break;
