...
// Copy the data from the block we just read into the superblock in memory
	...
	dbprintf('j', "DfsOpenFileSystem -- FS_Valid: %d\n", sb.FS_Valid);
	dbprintf('j', "DfsOpenFileSystem -- FS_BlockSize: %d\n", sb.FS_BlockSize);
	dbprintf('j', "DfsOpenFileSystem -- FS_BlockCount: %d\n", sb.FS_BlockCount);
	dbprintf('j', "DfsOpenFileSystem -- FS_InodeBlock: %d\n", sb.FS_InodeBlock);
	dbprintf('j', "DfsOpenFileSystem -- FS_InodeCount: %d\n", sb.FS_InodeCount);
	dbprintf('j', "DfsOpenFileSystem -- FS_FBVBlock: %d\n", sb.FS_FBVBlock);
  ...
  fbv[j/32] |= 1<<(j%32);
  j++;
  dbprintf('j', "DfsOpenFileSystem -- 1 file system block used for MBR/Superblock\n");
  ...
  dbprintf('j', "DfsOpenFileSystem -- %d file system blocks used for inodes\n", (64*sb.FS_InodeCount/sb.FS_BlockSize));
  ...
  dbprintf('j', "DfsOpenFileSystem -- %d file system blocks used for free block vector\n", (sb.FS_BlockCount/(sb.FS_BlockSize*8)));
...
// Read inodes
  																											...
  																											// used to hold inodes
  {
  	...
  	{
  		dbprintf('j',"DfsOpenFileSystem -- DfsReadBlock faoiled attempting to read inode #%d\n", i);
  		return(DFS_FAIL);
  	}
	...
	if ((64*sb.FS_InodeCount) % sb.FS_BlockSize){
		...
		dbprintf('j',"DfsOpenFileSystem -- NUMBER OF INODES DOES NOT DIVIDE EVENLY INTO FS_BLOCKSIZE!\n");
...
// Read free block vector
  ...
  {
  	dbprintf('j',"DfsOpenFileSystem -- FBV read, Iteration #%d\n", i);
	...
	if (sb.FS_BlockCount%(sb.FS_BlockSize*8)){
		...
		dbprintf('j',"DfsOpenFileSystem -- NUMBER OF FBV BITS DOES NOT DIVIDE EVENLY INTO FS_BLOCKSIZE!\n");
...
int DfsCloseFileSystem() {
  ...
  for(i=0;i<(64*sb.FS_InodeCount/sb.FS_BlockSize);i++)	// Iterates through the number of file system blocks
  																											// used to hold inodes
  {
  	dbprintf('j',"DfsCloseFileSystem -- Inode writeback, Iteration #%d\n", i);
  	bzero((char *)&temp_fsblock, sizeof(temp_fsblock)); // Clear temp_fsblock
	...
	if ((64*sb.FS_InodeCount) % sb.FS_BlockSize){
		...
		dbprintf('j',"DfsCloseFileSystem -- NUMBER OF INODES DOES NOT DIVIDE EVENLY INTO FS_BLOCKSIZE!\n");
  ...
  for(i=0;i<(sb.FS_BlockCount/(sb.FS_BlockSize*8));i++) // Iterates through the number of file system blocks
  																											// used to hold the FBV
  {
  	dbprintf('j',"DfsCloseFileSystem -- FBV writeback, Iteration #%d\n", i);
	...
	if (sb.FS_BlockCount%(sb.FS_BlockSize*8)){
		...
		dbprintf('j',"DfsCloseFileSystem -- NUMBER OF FBV BITS DOES NOT DIVIDE EVENLY INTO FS_BLOCKSIZE!\n");
...
int DfsCacheFlush()
{
	...
	for( k= 0; k < items; k++) //TODO Validate if working
	{
		...
		{
			dbprintf('C',"DfsCacheFlush: could not save to start of full_sltos queue\n");
			return (DFS_FAIL);
		 }
		if (AQueueRemove (&toBeRemoved) == QUEUE_FAIL)
		{
...
int DfsInodeFilenameExists(char *filename) {
	...
	printf("DfsInodeFilenameExists: ERROR Filename not found\n");
	return(DFS_FAIL);
}
	...
	else
	{
		...
		for(i=0; i< sb.FS_InodeCount; i++)
		{
			if (inodes[i].InUse == 0)
			{
				...
				//TODO just allocate indirrect here and never worry
				return(i);
			}
		}
...
int DfsInodeReadBytes(uint32 handle, void *mem, int start_byte, int num_bytes)
{
	...
	{
		dbprintf('D',"ONLY ONE BLOCK TO READ!!\n");
...
int DfsInodeWriteBytes(uint32 handle, void *mem, int start_byte, int num_bytes)
{
	...
	if (start_FS_Block == end_FS_Block)
	{
		dbprintf('d',"ONLY ONE BLOCK NEEDED!!\n");
		physicalBlockNum = DfsInodeTranslateVirtualToFilesys(handle,start_FS_Block);
...
int DfsInodeAllocateVirtualBlock(uint32 handle, uint32 virtual_blocknum) {
	...
	if (virtual_blocknum < 10)
	{
		...
		dbprintf( 'D',"DfsInodeAllocateVirtualBlock Virtual block %d is not used Allocating...\n ",virtual_blocknum);
		...
		//TODO WRITE zeros to this so that the association is made
		dbprintf( 'D',"SUCCESS DfsInodeAllocateVirtualBlock %d = ",virtual_blocknum);
		dbprintf( 'D'," has been allocated %d\n",physicalBlockNum);
	...
	else // indirect addressing required
	{
		...
		printf("DfsInodeAllocateVirtualBlock -- Read Indirect table at Block #%d\n",inodes[handle].IndirectAddrTrans);
		if	(DfsReadBlock(inodes[handle].IndirectAddrTrans, &indirectTranslator) == DFS_FAIL)
		{
		...
		physicalBlockNum = DfsAllocateBlock(); // JSM
		//TODO Write zeros to the block to make sure asssociation
		if(physicalBlockNum == DFS_FAIL)
		{
			printf("ERROR DfsInodeAllocateVirtualBlock -- Unable to allocate physical block!!\n");
			return(DFS_FAIL);
		...
		printf("DfsInodeAllocateVirtualBlock -- Indirect table got! And Got block# %d for virtual\n",physicalBlockNum);
...
int DfsInodeTranslateVirtualToFilesys(uint32 handle, uint32 virtual_blocknum)
{
	...
	else // Must use indirect translation
	{
		if(inodes[handle].IndirectAddrTrans == 0)
		{
			printf("ERROR DfsInodeTranslateVirtualToFilesys : ERROR Can't TranslateN IndirectAddrTrans == 0!!\n");
			return(DFS_UNALLOC);
		}
		...
		if (result == 0)
		{
			printf("DfsInodeTranslateVirtualToFilesys -- Unallocated Indirect translation of VB#%d resulted in 0!!\n", virtual_blocknum);
			return(DFS_UNALLOC);
		}
