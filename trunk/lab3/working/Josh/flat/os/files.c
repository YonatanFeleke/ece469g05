...
int FileOpen(char *filename, char *mode){
	...
	{
		dbprintf('F', "FileOpen -- Opening file in RW mode\n");
	...
	{
		dbprintf('F', "FileOpen -- Opening file in R mode\n");
		// get inode_handle (create if doesn't exist)
		inode_handle = DfsInodeOpen(filename);
		if(inode_handle == -1)
		{
	...
	{
		dbprintf('F', "FileOpen -- Opening file in W mode\n");
		...
		dbprintf('F', "FileOpen -- File doesn't exist yet, creating file...\n");
		// then open
		inode_handle = DfsInodeOpen(filename);
		if(inode_handle == -1)
		{
...
int FileRead(int handle, void *mem, int num_bytes)
{
	...
	{
		printf("FileRead -- invalid num_bytes!\n");
		return(FILE_FAIL);
	}
...
int FileWrite(int handle, void *mem, int num_bytes)
{
	...
	if(num_bytes < 0 || num_bytes > FILE_MAX_READWRITE_BYTES)
	{
		printf("FileWrite -- invalid num_bytes!\n");
		return(FILE_FAIL);
	}
...
int FileDelete(char *filename)
{
	...
	// Invalidate any valid file descriptors still corresponding to this filename
	for(i=0; i<FILE_MAX_OPEN_FILES; i++)
	{
		if((fd_array[i].FD_Valid == 1) && (dstrncmp (filename, fd_array[i].FD_FileName, dstrlen(filename)) == 0))
		{
			// Populate file descriptor with appropriate data
			dbprintf('F', "FileDelete -- Invalidating file descriptor #%d ",i);
			dbprintf('F', "corresponding to filename: %s\n", filename);

			fd_array[i].FD_Valid = 0;
		}
	}


	dbprintf('F',"FileDelete -- Successfully deleted file: %s\n", filename);
	return(FILE_SUCCESS);
