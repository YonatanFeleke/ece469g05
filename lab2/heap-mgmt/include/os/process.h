...
typedef struct PCB {
  ...
  uint32	pagetable[MEM_L1_PAGE_TABLE_SIZE]; // Statically allocated page table  // JSM added static page table size
  uint32	num_p_pages;
  Link		*l;		// Used for keeping PCB in queues
  uint32	HeapFreeMap[MEM_MAX_HEAP_FREEMAP]; // array of ints that track the currently allocated/unallocated words in the heap
  uint32	HeapValidPtrMap[MEM_MAX_HEAP_FREEMAP]; // JSM -- array of ints indicating which words correspond to valid pointers to allocated chunks
  //uint32	HeapPtrSizes[MEM_MAX_HEAP_POINTER_ARRAY];//start addr and size // JSM -- not needed?
  //uint32	*userHeapArea;  // JSM -- not needed?
} PCB;
