...
void MemoryModuleInit() {
	...
	//while(1); // JSM REMOVE THIS ONCE WE CAN SUCCESSFULLY GET TO HERE!
...
uint32 MemoryTranslateUserToSystem (PCB *pcb, uint32 addr) {
	...
	dbprintf('m', "MemoryTranslateUserToSystem-Address to translate: 0x%.8X\n", addr);
	...
	if ((pte_idx & MEM_PTE_VALID) == 0) //not valid --> means the virtual adress isn't assigne
	{
		printf("MemoryTranslateUserToSystem- Josh is wrong...  Finally YES!\n");
		MemoryPageFaultHandler(pcb); //since the page table entry is not valid
		while(1);
		return (0);// the address is invalid
	}

	//pte_idx = pte_idx & 0xFFFFF000;
...
// out.
//---------------------------------------------------------------------
int MemoryPageFaultHandler(PCB *pcb)// attempting to translate a virtual address that is invalid (not set in physical)
{
  uint32 newpage;
  uint32 idx;
  dbprintf('m',"Stack Fault: 0x%.8X   UserStackPointer: 0x%.8X!!\n",pcb->currentSavedFrame[PROCESS_STACK_FAULT] ,pcb->currentSavedFrame[PROCESS_STACK_USER_STACKPOINTER]);
	if (pcb->currentSavedFrame[PROCESS_STACK_FAULT] <= MEM_MAX_VIRTUAL_ADDRESS) // NOT NEEDED because different trap is called and exits
		// is the variable that will only be pushed on a seg fault if so this will be a null pointer access
	{
		...
		{
			while (1)	{	dbprintf('m',"just tried to hack me U LOSE!"); }
			return(MEM_FAIL);
		}
	}
	else
