...
static uint32 pt_freemap[MEM_FREEMAP_PT_SIZE_IN_WORDS];
...
uint32 MemoryTranslateUserToSystem (PCB *pcb, uint32 addr) {
	...
	idx2 = (addr & MEM_L2_ADDRESS_MASK) >> (MEM_L2FIELD_FIRST_BITNUM);// get rid of lower 12 bit offset
	dbprintf('m', "MemoryTranslateUserToSystem-Index1: 0x%.8X Index2: 0x%.8X\n", idx1,idx2);
	L2_Table_Addr = (uint32 *)pcb->pagetable[idx1];
	//multiArrayAddr = ((uint32 *)(pcb->pagetable[idx1]) + (uint32 *)idx2);
	dbprintf('m', "MemoryTranslateUserToSystem-L2_Table_Addr: 0x%.8X\n", (uint32) L2_Table_Addr);
	...
	{
		printf("MemoryTranslateUserToSystem- INVALID PAGE TRANSLATION level!!  SEG FAULT!!\n");
		printf("Exiting Simulator!!\n\n");
		exitsim ();
	}
...
// out.
//---------------------------------------------------------------------
// TODO CHANGE FOR 2 LEVEL Page fault
