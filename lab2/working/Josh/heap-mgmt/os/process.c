...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  pcb = (PCB *)AQueueObject(AQueueFirst (&freepcbs)); //pull an entry from pcbs array and update it.
  ...
  for(i = 0; i < 4; i++)
  {
		...
		pcb->pagetable[i] = ((newpage*MEM_PAGE_SIZE) | MEM_PTE_VALID);
  }
  	///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (j=0; j<MEM_MAX_HEAP_FREEMAP; j++){
    	pcb->HeapFreeMap[j] = 0xFFFFFFFF;//empty
    }
    for (j=0; j<MEM_MAX_HEAP_FREEMAP; j++){
    	pcb->HeapValidPtrMap[j] = 0;//empty
    }
    /*
    for (j =0; j<MEM_MAX_HEAP_POINTER_ARRAY; j++){
		...
		*/
		...
		dbprintf('p', "ProcessFork-Allocating physical page #%d (Address 0x%.8X) for process virtual page #%d (Heap)\n\n", newpage, (newpage*MEM_PAGE_SIZE), i);
		pcb->pagetable[4] = ((newpage*MEM_PAGE_SIZE) | MEM_PTE_VALID); // JSM -- heap = 4th VIRTUAL PAGE per process

		/* JSM -- not needed for L1 page table?
  	//address for catch in Memory translate. UserHeap Address =0x4000 =
  	pcb->userHeapArea = (uint32 *)(newpage*MEM_PAGE_SIZE);
  	dbprintf('p', "Heap area is at physical address:0x%.8X   L2 Address: 0x%.8X\n", (int)(newpage*MEM_PAGE_SIZE), pcb->pagetable[i]);
  	dbprintf('p', "Contents at 0x%.8X: 0x%.8X\n\n", (int) &(pcb->pagetable[i]), pcb->pagetable[i]);
  ...
  // Allocate page for user stack
	...
	dbprintf('p', "ProcessFork-Allocating physical page #%d (Address 0x%.8X) for process virtual page #%d (user stack)\n\n", newpage, (newpage*MEM_PAGE_SIZE), MEM_L1_PAGE_TABLE_SIZE-1);
	pcb->pagetable[MEM_L1_PAGE_TABLE_SIZE-1] = ((newpage*MEM_PAGE_SIZE) | MEM_PTE_VALID); //255 -> physical
